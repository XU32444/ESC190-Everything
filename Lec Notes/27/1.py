# Adjacency matrix: M[i,j] is 1 if nodes i and j are linked
# nodes can be airports: "YYZ" (3), "YVR" (2), "YUL", "Whitehouse"(0)
#
# Whitehourse [0], YUL[1],
# 0
# 1
...
# Average-case access for dicts is O(1) (bu worst-case is O(n), n = num of nodes)
# class node:
#       def __init__(self,value):
#           self.value = value
#           slef.neighbours = set() # O(1) to look up and insert on average
# graph = [Nde("YYZ"),Node(YUL)]
# To make look-up efficient, turn graph into a dictionary (use nodes_dict)
